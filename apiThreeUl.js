...
async function getNames(softwareEngineers, key){
    ...
    let names = nameArray(ans, key);
    //console.log('names pls', names);
    return names;
}


function nameArray(data, key) {
    let ans = [];
...
async function summonList(names, key){ 
    getNames(names, key).then(namesList => {
...
summonList(softwareEngineers, 'name');
summonList(softwareEngineers, 'bio');
summonList(softwareEngineers, 'id');
